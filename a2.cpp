...
void Print_Consumer(Consumer* consumer, int number, Household* household) {
    ...
    std::cout << std::right << std::setw(COLUMN_WIDTH) << number << ": " << consumer->description
              << " (at address: " << consumer << ")" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "power consumption: " << std::left << std::fixed << std::setprecision(2) << consumer->watt << " W" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "power consumption standby: " << std::left << std::fixed << std::setprecision(2) << consumer->watt_standby << " W" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "annual hours of use: " << std::left << std::fixed << std::setprecision(2) << Annual_Hours_Of_Use(*consumer) << " h" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "annual hours of standby: " << std::left << std::fixed << std::setprecision(2) << Annual_Hours_Of_Standby(*consumer) << " h" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "annual consumption: " << std::left << std::fixed << std::setprecision(2) << Annual_Kwh(*consumer) << " kWh" << std::endl;
    std::cout << std::right << std::setw(COLUMN_WIDTH) << "annual costs: " << std::left << std::fixed << std::setprecision(2) << (Annual_Kwh(*consumer) * price) << " EUR" << std::endl;
}
...
Household* Copy_Consumers(const Household* fromHouse, Household* toHouse) {
    Household* tempHousehold = new Household(*toHouse);
    tempHousehold->consumers = nullptr;
    Consumer* fromConsumer = fromHouse->consumers;
    while (fromConsumer != nullptr)
        {
            Consumer *new_consumer = new Consumer(*fromConsumer);
            new_consumer->next = tempHousehold->consumers;
            tempHousehold->consumers = new_consumer;
            fromConsumer = fromConsumer->next;
        }

    *toHouse = *tempHousehold;
    delete tempHousehold;
...
int main()
{
    ...
    int n_households;
    int n = 0;
    float PRICE_KILO_WATT = household->price * int(100);
    ...
    while (opt != 'q')
    {
        ...
        case 'n':
        {
            ...
            if (households_arr[n - 1] == nullptr)
            {
                ...
                households_arr[n - 1]->city = household->city;
                households_arr[n - 1]->price = household->price;
